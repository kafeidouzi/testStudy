...
2. build 文件夹
   ...
   - webpack.dev.conf.js (webpack 开发环境配置) 的作用
...
1. 合理的 title，description，keywords:搜索对着三项的权重逐个减小，title 值强调重点即可。重要关键词出现不要出现超过两次，且要靠前，不同页面的 title 要有所不同；description 把页面内容高度概括，长度合适，不可过问关键词堆砌，不同 description 有所不同；keywords 列举出重要关键词即可
...
1. CSRF：跨站请求伪造，可以理解为攻击者盗用了用户身份，亿用户的身份去修改用户登录网站的一些信息。（防御： 使用验证码，使用 token，检查 https 头部的 referer（它记录了该 http 请求的来源地址）），或在 http 头部自定义属性并验证
...
- **身份认证** [https://blog.csdn.net/swadian2008/article/details/104285921]
  ...
  3. SSL 认证安全性较高，但是需要承担证书费用
     （客户端请求-> 服务端要求出示证书 -> 客户端发送证书 ->验证证书合法性，取出公钥->服务端随机生成随机数（对称秘钥），使用非对称算法和证书公钥加密，发送给客户端 -> 客户端使用非对称算法和证书私钥获取服务端发送的对称秘钥）
     **可防止泄露，假冒，请求重放**
  4. HTTP 表单验证 / 表单验证不存在 http 规范，表单验证一般配合 cookie 和 sessionId 的使用。 （用户将用户名，密码发送至服务端，服务端通过验证后，将 sessionId 返回给浏览器，浏览器会保存 sessionId 保存至 cookie。下次 QINGQIU 携带 sessionId,服务器进行验证）
- **状态管理**
  1. 由于 http 是无状态的，当一个请求得到服务器应答后，这次的交互信息就丢了，再次发送请求时，服务器就无法知道是否与上次有关。
     **解决方法有两种**
  - 第一种，每次应答都返回完整状态，让浏览器把它作为下一下请求的的一部分发送回来
  - 第二种，将其存在服务器某个地方，只发送回一个标识符，浏览器下次吧标识符发送回来
    **这两种方法中，信息可以通过三种方式之一发送给浏览器 1️⃣ 作为 cookie，2️⃣ 嵌入隐藏的 html 表单中 3️⃣ 附加在主体的 url 中**
- **Cache 缓存等机制相关的请求头和响应头**
  1. Cache-Control: xxxxx
  - no-store 告诉浏览器任何情况都不使用缓存
  - no-cache 请求或响应不能缓存
  - max-age = 30 表示缓存最长时间 30s
  - public 相应可以被任何缓存区缓存
  - private 对于单个用户的整个或部分响应消息
  2. expires 告诉浏览器在该时间段内有效（客户端时间改变可能会引发失效）
  3. Pragma :no-cache http 1.0 的，作用同 http1.1Cache-Control：no-cache
  4. Last-Modified：告知实体最后被修改的时间，用于浏览器校验对应缓存是否可用，与其相关有两个请求标头;
  - if-Modified-Since （如果被修改）
    只在 get 方法中有意义，如果实体在指定时间后没有修改，则返回有个 304，否则就正常 200，返回数据。如果只是 304 是好的，因为它只是检验了一下
  - if-Unmodified-Since （如果没被修改）get 方法除外
    1. 如果在某个时间点后，没有被修改，则继续请求，正常 200
    2. 如果在某个时间点后，已经被修改，则返回 412 Pre
...
- 前端发送请求，ios 端接收到请求，根据请求匹配的方法作相应处理，然后返回给 h5，h5 根据 ios 端返回的字符串，根据提前约定好的规则方法进行处理相关信息
...
### js 不用 var 声明，都是为全局变量

function show(){
var a=5,b=6,c=7;
}

### 浏览器输入网址之后发生了什么？------url 全过程

1. DNS 解析 （缓存---服务器）
   浏览器查看自己的缓存，没有；查看本地 host 文件，  
   （浏览器缓存-->系统缓存-->路由缓存）有则展示页面，如果缓存没有，则向域名服务器发起请求，解析域名(域名--->IP)
2. http 请求 （tcp 链接，三次握手）

- 客户端向服务器发送连接请求报文
- 服务器接收请求后回复 ACK 报文，并为这次链接分配资源
- 客户端在接受到 ACK 报文后，也向服务器发送 ACK 报文，并分配资源

3. 浏览器收到请求返回响应头响应体（数据）
4. 页面渲染 (浏览器收到服务器发送的响应头和响应体，进行客户端渲染，生成 Dom 树、解析 css 样式、js 交互。)

### watch 和 computed

1. computed 在初始化的時候就可以监听到，它是以有依赖的，可以依赖多个（多对一），不能操作异步。

2. watch 只有在数据变化时才回触发，它是一对多的，可以处理异步。
...
1. 减少 http 请求
2. 删除不必要的注释，空格之类的，开启 GZIP 压缩文件，结合 http 缓存
3. 减少 css 关键词数量
4. script 标签优化使用 async，defer ，使用预加载（prefetch&preload），dns 预解析 （dns-prefetch）
5. 减少不必要的重绘和回流 （间可能在 dom 树末端）
6. 图片懒加载
...
1. 热更新 hotModulesReplacement 文件修改后，webpack 重新打包，并将改动的模块发送到浏览器，浏览器用新模块替换掉旧的模块，区市县局部更新而非页面整体刷新
2. 间隔符 path.narmalize

3. output 属性：｛
   path，会简单告诉你 webpack 生成文件输出位置
   publicPath（输出目录），多被一些 webpack 插件使用，在 html 文件以生产环境方式被构建的时候，更新 css 文件内的 url 地址
   filename,
   chunkFilename
   ｝
...
暂时性： 不设置过期时间，表示该 cookie 生命周期为浏览器会话期间，只要关闭浏览器窗口，cookie 就消失了。这种生命周期为浏览器会话期被称为会话 cookie。
永久性： 设置了过期时间，浏览器会把 cookie 保存在硬盘上，关闭再次打开浏览器，这些 cookie 依然有效，直至超过设定的过期时间。

### 箭头函数和普通函数区别

1. 箭头函数相当于匿名函数， （1，不写｛｝，直接返回；2，可以版含多种语句）
2. 箭头函数是匿名函数，不能作为构造函数，不能使用 new

- 匿名函数赋给变量，可以被 new;箭头函数不行

3. 箭头函数不绑定 arguments，取而代之用 rest 参数解决
4. 箭头函数不绑定 this，会捕获其上下文的 this，且 this 不会改变
5. 箭头函数没有原型属性
...
1. for of 有迭代器的都可以遍历，普通对象没有迭代器属性直接遍历会报错。可以获取健值（遍历数组、类数组对象，字符串、Set、Map 以及 Generator 对象）

2. for in 可以遍历对象，获取健名。还会比那里手动添加 的属性及原型链上的健

### react 的 componentUnWillUpdate 为什么在 componentDidUpdate 会不准确

### generator/yield 和 async/await

1. generator/yield 生成器对象石油 function \* 返回的，遵循可迭代协议，迭代器协议 ，生成器是对象是可迭代的

2. async/await 则是更加简洁，减少嵌套，代码更加直观；自带执行器，不需要手动调用 next 方法

generator 返回的是一个生成器对象｛value：‘’，done：true/false｝，async 则返回的是一个 promise,await 可以返回 promise 的 resolve、reject 的值

### 迭代器实现

每次调用 next 方法，都会返回一个对象{value:'',done:true/false}，
知道 done 为 true 停止下来

### loader 和 plugin 区别

1. loader 属于文件加载器，加载资源是对文件进行一些处理，诸如编译，压缩等，**loader 执行顺序是相反的**，即最后一个 loader 先执行，第一个最后执行。loader 只是为了打包。

2. plugin 是一个扩展器，他可以丰富 webpack，针对 loader 处理结束后，监听 webpack 的事件，在合适的时机通过 webpack 提供 api 改变输出结果。
   plugin 则是在整个编译周期都起作用

### slot 实现原理

1. 分为 匿名插槽，具名插槽 （#），作用域插槽

原理：vm 对象的\$slots 会存储组件的所有插槽的 vdom，主要通过解析 children，如果子组件指定了插槽名，会将 vdom push 到对应名称的数组中，否则会 push 到 default 的数组中。 2. 作用域插槽： 子插槽交自己的数据传给父组件
...
1. 相比于 mixin，hooks 是一个函数，可以更好的复用代码

### mvc,mvvm,mvp

1. mvc 视图层（触发一些指令给到 c），数据层（根据 c 的操作将数据给到 v），控制层（接受 v，或者自个的指令，进行复杂的逻辑操作，触发 m）
2. mvvm 与 mvp 差不多，将 p 换成了 vm，不同的是，它使用双向绑定会自动触发
3. mvp 斩断了 vm 的联系，多了个 p，p 和 v，p 和 m 之间都是双向的，
